...
pInterfaceDescriptionBody = do { linkType <- word16
                               ; reserved <- word16
                               ; snapLen <- word32
                               ; options <- many word32
...
pEnhancedPacketBody = do { interfaceID <- word32
...
pBlock = do { bType <- fmap byteSwap32 word32
