...
pTcpHeader = do { sp <- word16
                ...
                ; return $ TcpHeader dp sp seq ack
                  offset flags ws cs up
                } :: Parser TcpHeader

pTcpOptionalHeader offset = fmap TcpOptionalHeader (nBytes offset) :: Parser TcpOptionalHeader

pTcpPacket = do { th <- pTcpHeader
                ; oh <- let (TcpOffset len) = offset th
                        in if len > 5
                           then fmap Just (pTcpOptionalHeader $ (fromEnum len - 5) * 4)
                           else return Nothing
                ; td <- many anyByte
                ; return $ TcpPacket th oh td
                } :: Parser TcpPacket
